...
export default function ChangePasswordForm({ setShowModal, toastRef }) {
    ...
    const onSubmit = async() => {
        ...
        if (!resultUpdatePassword.statusResponse) {
            setErrorNewPassword("Hubo un problema combiando la contraseña, por favor intenta más tarde.")
            return
        }

        toastRef.current.show("Se ha actualizado la contraseña.", 2000)
        setShowModal(false)
    }
    ...
    return (
        <View style={styles.view}>
            ...
            />
            <Input 
                placeholder="Ingresa tu confirmación de la nueva contraseña."
                containerStyle={styles.input}
